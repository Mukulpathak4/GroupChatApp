...
async function DOMloadChat(grpid) {
    ...
    if (localChats == undefined) {
        const getAllChats = await axios.get("http://52.66.181.47:3000/chat/getAllChats",
...
async function getUpdatedChats(grpid) {
    ...
    if (grpid == 1) {
        const result = await axios.get(`http://52.66.181.47:3000/token/getGroupToken/${grpid}`,
            { headers: { "Authorization": token } });
    ...
    const updatedMsg = await axios.get(`http://52.66.181.47:3000/chat/getUpdate/${lastMsgId}`,
        { headers: { "Authorization": token } });
...
async function DOMloadGroups() {
    const token = localStorage.getItem("token");
    const getGroups = await axios.get("http://52.66.181.47:3000/chat/getAllGroups",
        { headers: { "Authorization": token } });
...
async function onGroupClick(e) {
    ...
    if (e.target.classList.contains("open")) {
        ...
        try {
            ...
            const result = await axios.get(`http://52.66.181.47:3000/token/getGroupToken/${groupId}`,
                { headers: { "Authorization": token } });
...
async function sendMsg() {
    ...
    try {
        ...
        await axios.post("http://52.66.181.47:3000/chat/send-msg", {
            msg: msg
        }, { headers: { "Authorization": token } })
...
async function createGroup() {
    try {
        ...
        if (!groupName) {
            ...
            const result = await axios.post("http://52.66.181.47:3000/chat/createGroup", { groupName: groupName }, { headers: { "Authorization": token } });
...
async function addmember() {
    ...
    if (member) {
        console.log(member);
        try {
            const result = await axios.post("http://52.66.181.47:3000/chat/addmember", {
                member: member
            }, { headers: { "Authorization": token } });
...
async function publicGroup() {
    try {
        const token = localStorage.getItem("token");
        const result = await axios.get("http://52.66.181.47:3000/token/getPublicToken",
            { headers: { "Authorization": token } });
        localStorage.setItem("token", result.data.token);
        window.location.href = "/home"
    }
...
async function showpopup() {
    ...
    try {
        const token = localStorage.getItem("token");
        const allMembers = await axios.get("http://52.66.181.47:3000/chat/viewAllMembers",
            { headers: { "Authorization": token } });
...
async function memberClick(e) {
    ...
    if (e.target.classList.contains("admin")) {
        try {
            const result = await axios.get(`http://52.66.181.47:3000/chat/addAdmin/${userid}`,
                { headers: { "Authorization": token } });
        }
        catch (err) {
            console.log(err)
        ...
        try {
            const result = await axios.get(`http://52.66.181.47:3000/chat/removeMember/${userid}`,
                { headers: { "Authorization": token } });
        }
        catch (err) {
            console.log(err)
...
async function deleteGroup(groupid) {
    ...
    try {
        ...
        await axios.delete(`http://52.66.181.47:3000/chat/deleteGroup/${groupid}`, {
            headers: { "Authorization": token }
        });
